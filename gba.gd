...
@export var gameboy_pov = preload("res://gameboy_pov.tscn")
@onready var sfx = preload("res://addons/kenney_ui_audio/switch26.wav")
...
func _on_Area2D_input_event(viewport, event, shape_idx):
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		get_tree().change_scene_to_packed(gameboy_pov) # Switch to gameboy pov
...
func _ready():
	...
	$"gba_normal".visible = true
	$"gba_hovered".visible = false

# This function is called when the mouse enters the Area2D
func _on_Area2D_mouse_entered():
	print("Mouse entered")
	$"gba_normal".visible = false
	$"gba_hovered".visible = true
