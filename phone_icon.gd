...
@export var phone_inside = preload("res://phone.tscn")
@onready var sfx = preload("res://audio/mixkit-light-button-2580.wav")
...
func _on_Area2D_input_event(viewport, event, shape_idx):
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		get_tree().change_scene_to_packed(phone_inside) # Go inside phone
